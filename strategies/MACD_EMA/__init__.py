...
class MACD_EMA(Strategy):
    @property
    def macd(
            self):  # this returns: macd, signal and hist which can be referenced as self.macd[0], self.macd[1] and self.macd[2], respectively
    ...
    def hyperparameters(
            self):  # This is set up for optimization but if you just want to backtest with your own values then change the default value only.
        return [
